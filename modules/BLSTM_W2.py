...
class BLSTM_W2(object):
    ...
    def predict(self, model, sentence):
        ...
        pred = model.predict([tokens_np, casing], verbose=False)[0]
    ...
    def buildModel(self):
        ...
        # set up model
        self.model = Model(inputs=[words_input, casing_input], outputs=[output])
        ...
        if self.verify_model_exists() is False:
            ...
            for epoch in range(self.epochs):
                ...
                for i, batch in enumerate(iterate_minibatches(self.train_batch, self.train_batch_len)):
                    ...
                    self.model.train_on_batch([tokens, casing], labels)
